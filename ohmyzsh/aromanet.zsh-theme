...
elif [ "$POWERLINE_RIGHT_A" = "exit-fail" ]; then
...
POWERLINE_BEGIN_LINE=""
if [ "$POWERLINE_USER_NAME" != "" ]; then
  POWERLINE_BEGIN_LINE="$POWERLINE_SEC1_BG$POWERLINE_SEC1_TXT $POWERLINE_USER_NAME %k%f$POWERLINE_SEC1_FG%K{blue}"$'\ue0b0'
fi

PROMPT=$POWERLINE_BEGIN_LINE"%k%f%F{white}%K{blue} "$POWERLINE_CURRENT_PATH"%F{blue}"$POWERLINE_GIT_INFO_LEFT" %k"$'\ue0b0'"%f "
